...
let () =
  let config = Sudoku_config.create () in

  let input_board = Sudoku_config.get_input_board config in
  ...
  let num_rows = Sudoku_board.get_num_rows input_board in
  let num_cols = Sudoku_board.get_num_cols input_board in
  let maxRowIndex = num_rows - 1 in
  let maxColIndex = num_cols - 1 in

  let numEntries = num_rows * num_cols in

  let position2Var = Hashtbl.Poly.create ~size:numEntries () in
  ...
  let output_filename = Sudoku_config.get_output_file config in
